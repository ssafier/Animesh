    ...
    dataserver(key query_id, string data) {
        ...
        if (data != "") {
            ...
            if (cmd == "MENU") {
                ...
                integer colorIx;
                integer ix;
                for (; ix < MAX_BALLS; ++ix) { // for each possible ball
                    ball_color = llList2String(ldata, ix + 2); // get next color name from config
                    colorIx = llListFindList(Colornames, (list)ball_color);
                    if (colorIx != -1) {
                        colors += (colorIx << (4 * ix));    // 4 = bits per color (16 colors)
                    }
                }

                menus = (menus=[]) + menus + (list) arg1;
                balls = (balls=[]) + balls + (list) colors;
                buttonindex = (buttonindex=[]) + buttonindex + (list) CurButtonIx;
                users = (users=[]) + users + (list) auth;
...
//                }
                ...
                if (b0 == 12 && llGetListLength(menus) > 1) {
                    ...
                    if (AutoBack) {
                        ix = -2;
                         // Add a "BACK" button
                        buttons  = llListInsertList(buttons, (list)"BACK", ix);
                        commands = llListInsertList(commands, (list)"BACK", ix);
                        ++CurButtonIx;
                    }
                    buttons  = llListInsertList(buttons, (list)"More-->", ix);
                    commands = llListInsertList(commands, (list)"MORE", ix);
                    ++CurButtonIx;
                    b0 = -ix;
                }
                if (cmd == "POSE") {
                    ...
                    LMButtons = (LMButtons = []) + LMButtons + arg1;
                    LMParms = (LMParms=[]) + LMParms + llList2String(ldata, 1);
                } else if (cmd == "SOUND") {
                    SoundNames += (list) arg1;
                    Sounds += (list) llList2String(ldata, 1);
                }
                commands = (commands=[]) + commands + (list) cmd;
                buttons = (buttons=[]) + buttons + (list) arg1;
                ++CurButtonIx;
                ++b0;
            }
        }
    ...
    state_exit() {
        buttonindex = (buttonindex=[]) + buttonindex + (list) CurButtonIx;    //enter last buttonindex
        commands = (commands=[]) + commands + (list) "";            //empty command for undefined buttons (-1)
