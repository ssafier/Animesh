//MPLV2 by Learjeff Innis, based on
...
state on {
    state_entry() {
        ...
        if (llGetInventoryType("~swap") == INVENTORY_SCRIPT) {
            llSetScriptState("~swap", TRUE);
            llResetOtherScript("~swap");
        }
    ...
    link_message(integer from, integer num, string str, key dkey) {
	    ...
	    string data = get_pose_data(name);
	    llMessageLinked(LINK_THIS, 0, "POSEPOS", (key) data);  // to ~pos
            return;
        }
        ...
        if (str == "OK?") {                                 //question from menu, before loading menu
            ...
            integer ix;
            for (ix = 0; ix < PosCount; ++ix) {
                string name = llList2String(Poses, ix);
                llOwnerSay("{" + name + "} " + get_pose_data(name));
            }
