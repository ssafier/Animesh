...
default {
  ...
  link_message(integer from, integer chan, string msg, key xyzzy) {
    ...
    case WANDER: {
      ...
      vector p1 = (vector)(string) params[0];
      vector p2 = (vector)(string) params[1];
      g_moving = TRUE;
      g_animation = (string) params[2];
      g_pick_next_target = TRUE;
      if (p1.x < p2.x) {
	g_min.x = p1.x;
	g_max.x = p2.x;
      } else {
